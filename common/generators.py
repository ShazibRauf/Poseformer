...
class ChunkedGenerator:
    ...
    def __init__(self, batch_size, cameras, poses_3d, poses_2d,
        ...
        for x in range(len(self.poses_2d)):
            N = N+len(self.poses_2d[x])
            for y in range(len(self.poses_2d[x])):
                ls.append(self.poses_2d[x][y])
                
        from sklearn.decomposition import PCA
        self.pca = PCA()
        self.pca.fit(np.array(ls).transpose(0, 2, 1).reshape(-1, 2*17))
            ...
            for b_i in range(start_idx, self.num_batches):
                ...
                if self.poses_3d is None and self.cameras is None:
                    ...
                    if (self.stage == 1):
                        ...
                        yield self.batch_cam[:len(chunks)], self.batch_3d[:len(chunks)], self.batch_2d[:len(chunks)], chunks
            if self.endless:
                self.state = None
            else:
                enabled = False
...
class ChunkedGenerator_stage3:
    ...
    def __init__(self, batch_size, cameras, poses_3d, poses_2d,
                 ...
                 shuffle=True, random_seed=1234,
                 augment=False, kps_left=None, kps_right=None, joints_left=None, joints_right=None,
                 endless=False, pairs=None, elevation=None):
        assert poses_3d is None or len(poses_3d) == len(poses_2d), (len(poses_3d), len(poses_2d))
        assert cameras is None or len(cameras) == len(poses_2d)
        ...
        self.batch_2d = np.empty((batch_size, chunk_length + 2*pad, poses_2d[0].shape[-2], poses_2d[0].shape[-1]))
        self.batch_elevation = np.empty((batch_size*(chunk_length + 2*pad), 1))
        ...
        while enabled:
            ...
            for b_i in range(start_idx, self.num_batches):
                ...
                for i, (seq_i, start_3d, end_3d, flip) in enumerate(chunks):
                    ...
                    seq_2d = self.poses_2d[seq_i]
                    seq_3d = self.poses_3d[seq_i]
                    ele = self.elevation[seq_i]
                    low_2d = max(start_2d, 0)
                    high_2d = min(end_2d, seq_3d.shape[0])
                    pad_left_2d = low_2d - start_2d
                    pad_right_2d = end_2d - high_2d
                    if pad_left_2d != 0 or pad_right_2d != 0:
                        self.batch_2d[i] = np.pad(seq_2d[low_2d:high_2d], ((pad_left_2d, pad_right_2d), (0, 0), (0, 0)), 'edge')
                    ...
                    num_frames = self.chunk_length + 2*self.pad
                    self.batch_elevation[i*num_frames:i*num_frames+num_frames] = ele[start_3d][0]
                    ...
                    if self.poses_3d is not None:
                        ...
                        seq_2d = self.poses_2d[seq_i]
                        low_3d = max(start_3d, 0)
                        high_3d = min(end_3d, seq_2d.shape[0])
                        pad_left_3d = low_3d - start_3d
                        pad_right_3d = end_3d - high_3d
                        if pad_left_3d != 0 or pad_right_3d != 0:
                            #self.batch_3d[i] = np.pad(seq_3d[low_3d:high_3d], ((pad_left_3d, pad_right_3d), (0, 0), (0, 0)), 'edge')
                            self.batch_2d[i] = np.pad(seq_3d[low_3d:high_3d], ((pad_left_3d, pad_right_3d), (0, 0), (0, 0)), 'edge')
                        else:
                            #self.batch_3d[i] = seq_3d[low_3d:high_3d]
                            self.batch_2d[i] = seq_2d[low_3d:high_3d]
                ...
                if self.poses_3d is None and self.cameras is None:
                    ...
                    yield self.batch_cam[:len(chunks)], self.batch_3d[:len(chunks)], self.batch_2d[:len(chunks)], self.batch_elevation[:len(chunks)*(self.chunk_length + 2*self.pad)]
            if self.endless:
                self.state = None
            else:
                enabled = False
